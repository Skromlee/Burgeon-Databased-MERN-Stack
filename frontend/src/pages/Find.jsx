...
import { reset, getParcelById } from "../features/parcel/parcelSlice";
...
const Find = () => {
    ...
    const { parcelbyid, isSuccess, isError, message } = useSelector(
        (state) => state.parcels
    );
    ...
    useEffect(() => {
        ...
        if (parcelbyid.length > 0) {
            let arrivedDate = new Date(parcelbyid[0].createdAt);
            // console.log(arrivedDate, "-----");
            switch (parcelbyid[0].typeofshipment) {
                case "Normal":
            ...
            switch (isDelivered) {
                case "finish":
                    configImgPath = {
                        ...configImgPath,
                        400: "/status/400_P.png",
                    };
                    break;
                case "process":
                    configImgPath = {
                        ...configImgPath,
                        400: "/status/400_C.png",
                    };
                    break;
                case "false":
                    configImgPath = {
            ...
            switch (isOnDelivery) {
                case "finish":
                    configImgPath = {
                        ...configImgPath,
                        300: "/status/300_P.png",
                    };
                    break;
                case "process":
                    configImgPath = {
                        ...configImgPath,
                        300: "/status/300_C.png",
                    };
    ...
    }, [parcelbyid, isSuccess, "dispatch", "isError", "message"]);

    const [parcelId, setParcelId] = useState({ parcelId: "" });
    ...
    const handleSubmit = (e) => {
        ...
        if (parcelId.parcelId.length < 0) {
            toast.error("You must input Parcel ID");
        }
        dispatch(getParcelById(parcelId.parcelId));
    };
    ...
    return (
        <>
            <div>
                <form
                    className="mx-auto container h-64 flex items-center"
                    onSubmit={handleSubmit}
                >
                    <div className="flex mx-auto items-center space-x-4">
                        <div className="relative border-b-2 my-4 focus-within:border-brightRedLight w-96 mx-auto align-middle">
                            <input
                                type="text"
                                id="parcelId"
                                name="parcelId"
                        ...
                        <button
                            type="submit"
                            className="bg-brightRed h-fit text-white rounded-full p-2 px-4 hover:cursor-pointer hover:bg-brightRedLight"
                        >
                            Search
                        </button>
                    </div>
